...
class RewardRolloutWorker(Worker):
    ...
    def _build_rollout(self):
        ...
        infer_tp = self.config.rollout.tensor_model_parallel_size
        dp = self.world_size // infer_tp
        print("world size of the reward rollout: {}".format(self.world_size))
        assert self.world_size % infer_tp == 0, f'rollout world_size: {self.world_size} is not divisible by infer_tp: {infer_tp}'
        rollout_device_mesh = init_device_mesh('cuda', mesh_shape=(dp, infer_tp), mesh_dim_names=['dp', 'infer_tp'])
        rollout_name = self.config.rollout.name

        from verl.utils.model import get_generation_config
        from verl.workers.sharding_manager.fsdp_vllm_reward import FSDPVLLMRewardShardingManager
        from verl.workers.rollout.vllm_rollout import vLLMRewardRollout, vllm_mode
        log_gpu_memory_usage(f'Before building {rollout_name} for reward rollout', logger=None)
        local_path = copy_to_local(self.config.rollout.model_name)
        ...
        rollout = vLLMRewardRollout(model_path=local_path,
                                    config=self.config.rollout,
                                    tokenizer=self.tokenizer,
                                    device_mesh=rollout_device_mesh)
        log_gpu_memory_usage(f'After building {rollout_name} for reward rollout', logger=None)
        if torch.distributed.get_world_size() == 1:
            self.config.rollout.load_format = 'dummy_hf'
    ...
    def init_model(self):
        ...
        log_gpu_memory_usage('After building rollout', logger=None)
