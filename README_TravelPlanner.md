...
│   └── attractions.csv
├── flights/
│   └── clean_Flights_2022.csv  # 注意：此文件较大，需要单独下载
├── restaurants/
...
**注意**：`clean_Flights_2022.csv` 文件由于超过GitHub文件大小限制，需要从原始数据源单独下载。

## 2. 数据预处理

使用以下代码格式处理TravelPlanner数据集：
...
运行命令：
```bash
python preprocess_travelplanner.py --local_dir /path/to/raw/data --save_dir data/Travelplanner
```

处理完成后，数据将保存为 `train.parquet` 和 `test.parquet` 文件，可直接用于训练。
